...
WEIGHT_PATH = "C:\\Users\\帅小伙\\Desktop\\mnistTorch\\ministCNN.wts"
ENGINE_PATH = "./mnistCNN.engine"
...
def create_engine(max_batch_size, builder, config, dt):
    ...
    conv1=network.add_convolution(input=data,
                                num_output_maps=32,
                                kernel_shape=(5,5),
                                kernel=weight_map["conv.0.weight"],
                                bias=weight_map["conv.0.bias"])
    assert conv1
    conv1.stride =(1,1)
    conv1.padding =(2,2)
    ...
    conv2 = network.add_convolution(input=pool1.get_output(0),
                                    num_output_maps=64,
                                    kernel_shape=(5,5),
                                    kernel=weight_map["conv.3.weight"],
                                    bias=weight_map["conv.3.bias"])
    assert conv2
    conv2.stride =(1,1)
    conv2.padding =(2,2)
    ...
    # 全连接
    fc=network.add_fully_connected(input=pool2.get_output(0),
                                num_outputs=10,
...
if __name__ == "__main__":
    ...
    #     print(
    #         "arguments not right!\n"
    #         "python cnnTensorRT.py -s   # serialize model to plan file\n"
    #         "python cnnTensorRT.py -d   # deserialize plan file and run inference"
    #     )
    #     sys.exit()
    ...
    # data = np.ones((BATCH_SIZE * 1 * INPUT_H * INPUT_W), dtype=np.float32)
    inputs, outputs, bindings, stream = allocate_buffers(engine)
    inputs[0].host = data

    trt_outputs = do_inference(context, bindings=bindings, inputs=inputs, outputs=outputs, stream=stream)

    max_value = np.max(trt_outputs)
    print(max_value)
    max_index = np.argmax(trt_outputs)
    print(f'Output: \n{trt_outputs[0][:10]}\n{trt_outputs[0][-10:]}')
    print("识别结果为:{}".format(max_index))
